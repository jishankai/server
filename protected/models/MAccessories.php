 ...
 */
class MAccessories extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return MAccessories the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('playerId, acId, quality, createTime, updateTime', 'required'),
			array('acId, quality, level', 'numerical', 'integerOnly'=>true),
			array('playerId, createTime', 'length', 'max'=>10),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, playerId, acId, quality, level, createTime, updateTime', 'safe', 'on'=>'search'),
		);
	}
